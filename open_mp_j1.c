...
/*	struct timespec start_time;
	...
	do {
		...
		#pragma omp for private (i, j) schedule (dynamic, 100)
		for (i = 0; i < nr_read_start/bpn; i+=dimension) {
			for (j = i+dimension; j < nr_read_start/bpn; j+=dimension) {
				dist = sqrtf((start_points[i]-start_points[j])*(start_points[i]-
						 ...
						 -start_points[j+2]));
				local_histogram[(int)(dist*100)] += 1;
			}
		}
		#pragma omp critical
		{
		...
		do {
			...
			#pragma omp for private (i, j) schedule (dynamic, 100)
			for (i = 0; i < nr_read_start/bpn; i+=dimension) {
				for (j = 0; j < nr_read_end/bpn; j+=dimension) {
					dist = sqrtf((start_points[i]-end_points[j])*(start_points[i]-
